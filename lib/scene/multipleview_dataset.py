...
class multipleview_dataset(Dataset):
    ...
    def load_images_path(self, cam_folder, cam_extrinsics, cam_intrinsics, split):
        ...
        for idx, key in enumerate(cam_extrinsics):
            ...
            number = ((extr.name).split('.')[0])[5:]
            number_ls.append(int(number))
        number_ls = np.array(number_ls)
        number_idx = np.argsort(number_ls)
        for idx in number_idx:
            extr = cam_extrinsics[idx+1]
            R = np.transpose(qvec2rotmat(extr.qvec))
            T = np.array(extr.tvec)
            number = ((extr.name).split('.')[0])[5:]
            images_folder=os.path.join(cam_folder,"cam"+number.zfill(2))
            image_range=range(image_length)
            if split=='train' and number!='1':
                image_range = [image_range[0]]
            for i in image_range:    
                num=i+1
                image_path=os.path.join(images_folder,"frame_"+str(num).zfill(5)+".jpg")
                if not os.path.exists(image_path):
                    image_path=os.path.join(images_folder,"frame_"+str(1).zfill(5)+".jpg")
    ...
    def get_video_cam_infos(self,datadir):
        ...
        N_views = 300
        val_poses = get_spiral(poses, near_fars, N_views=N_views, rads_scale=1)

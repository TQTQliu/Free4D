...
import contextlib
from monst3r.dust3r.cloud_opt.base_opt import BasePCOptimizer, edge_str
from monst3r.dust3r.cloud_opt.pair_viewer import PairViewer
from monst3r.dust3r.utils.geometry import xy_grid, geotrf, depthmap_to_pts3d
from monst3r.dust3r.utils.device import to_cpu, to_numpy
from monst3r.dust3r.utils.goem_opt import DepthBasedWarping, OccMask, WarpImage, depth_regularization_si_weighted, tum_to_pose_matrix
from monst3r.third_party.raft import load_RAFT
from sam2.build_sam import build_sam2_video_predictor
import hydra
sam2_checkpoint = "lib/viewcrafter/extern/monst3r/third_party/sam2/checkpoints/sam2.1_hiera_large.pt"
model_cfg = "lib/viewcrafter/extern/monst3r/third_party/sam2/sam2/configs/sam2.1/sam2.1_hiera_l.yaml"
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def get_flow(self, sintel_ckpt=False): #TODO: test with gt flow
        ...
        flow_net = load_RAFT() if sintel_ckpt else load_RAFT("ckpt/sea_raft/Tartan-C-T-TSKH-spring540x960-M.pth")
    ...
    def refine_motion_mask_w_sam2(self):
        ...
        try:
            ...
            with torch.autocast(device_type=device, dtype=autocast_dtype):
                ...
                hydra.initialize_config_dir(version_base="1.3", config_dir=os.path.abspath("./lib/viewcrafter/extern/monst3r/third_party/sam2/sam2/configs/sam2.1/"))
                predictor = build_sam2_video_predictor("sam2.1_hiera_l", sam2_checkpoint, device=device)
    ...
    def get_im_poses(self):
        if self.batchify:
            ...
            ### Lock the poses for all frames 
            poses_tmp = self.get_im_poses_batch()
            return (poses_tmp).mean(0,keepdim=True).repeat(len(poses_tmp),1,1)
        else:
            # return self.get_im_poses_non_batch()
            poses_tmp = self.get_im_poses_non_batch()
            return (poses_tmp).mean(0,keepdim=True).repeat(len(poses_tmp),1,1)

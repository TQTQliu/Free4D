...
from lib.utils.general_utils import safe_state, multithread_write
from argparse import ArgumentParser
from configs.arguments import ModelParams, PipelineParams, get_combined_args, ModelHiddenParams
from lib.gaussian_renderer import GaussianModel
from time import time
            
to8b = lambda x : (255*np.clip(x.cpu().numpy(),0,1)).astype(np.uint8)
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type, save_gs=False):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        start = time()
        rendering = render(view, gaussians, pipeline, background, cam_type=cam_type, save_gs=save_gs, save_dir=model_path)["render"]
        if idx>0:
    ...
    print("Render FPS:", (len(views)-1)/render_time)
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, cam_type, save_gs=False)
        if not skip_test:

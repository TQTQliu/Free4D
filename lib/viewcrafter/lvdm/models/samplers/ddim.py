...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      x0_ref=None, replace_mask=None, viewmask=None, **kwargs):
        ...
        if viewmask is not None:
            viewmask = viewmask.permute(0,3,1,2)
            viewmask = F.interpolate(viewmask, img.shape[-2:])
            viewmask = viewmask[:,0]
            viewmask = viewmask[None,None].repeat(1,4,1,1,1)

        if replace_mask is not None:
            replace_mask = replace_mask.permute(0,3,1,2)
            replace_mask = F.interpolate(replace_mask, img.shape[-2:])
            replace_mask = replace_mask[:,0]
            replace_mask = replace_mask[None,None].repeat(1,4,1,1,1)
        ...
        for i, step in enumerate(iterator):
            ...
            if viewmask is not None:
                if i<0.25*total_steps:
                    mask_vis = (viewmask>0.99).float()
                    assert x0 is not None
                    img_orig = self.model.q_sample(x0, ts)
                    img = img_orig * mask_vis + (1. - mask_vis) * img # keep original & modify use img

            # ## use mask to blend noised reference latent (ref_orig) & new sampled latent (img)
            if replace_mask is not None and x0_ref is not None:
                thres = 0.5
                ref_orig = self.model.q_sample(x0_ref, ts) 
                mask_ref = (replace_mask>thres).float()
                img = ref_orig * mask_ref + (1. - mask_ref) * img
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            ## adaptive cfg
            if viewmask is not None:
                model_output1 = e_t_cond
                model_output2 = e_t_uncond + unconditional_guidance_scale * (e_t_cond - e_t_uncond)
                thres = 0.5
                mask_cfg = (viewmask>thres).type(torch.float32)
                model_output = mask_cfg * model_output1 + (1-mask_cfg) * model_output2
            else:
